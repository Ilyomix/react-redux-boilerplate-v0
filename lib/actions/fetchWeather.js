...
export function fetchWeather(town) {
  ...
  return dispatch => {
    ...
    fetch(reqUrl, reqSettings).then(res => res.json()).then(data => dispatch({ type: FETCH_WEATHER_SUCCESS, payload: data })).catch(err => dispatch({ type: FETCH_WEATHER_ERROR, payload: err }));
  };
}
